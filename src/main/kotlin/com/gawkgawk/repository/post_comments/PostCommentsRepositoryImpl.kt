...
import com.gawkgawk.dao.post_comments.PostCommentRow
import com.gawkgawk.dao.post_comments.PostCommentsDao
import com.gawkgawk.model.*
...
class PostCommentsRepositoryImpl(
    ...
    override suspend fun addComment(params: NewCommentParams): Response<CommentResponse> {
        ...
        return if (postCommentRow == null){
            Response.Error(
                ...
                data = CommentResponse(success = false, message = "Could not insert comment to the database")
            )
        } else{
    ...
    override suspend fun getPostComments(postId: Long, pageNumber: Int, pageSize: Int): Response<GetCommentsResponse> {
        val commentRows = commentsDao.getComments(
            postId = postId,
            pageNumber = pageNumber,
            pageSize = pageSize
            )

        val comments = commentRows.map { toPostComment(it) }
